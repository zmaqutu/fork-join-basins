...
do                                              #this is a while loop that reads from the file with split data and uses each line as an argume
        ...
        #IFS='_'
        #set $splitz
        make SequentialBasins "ARGS=data/large_in.txt SequentialTest.txt"
        count=`expr $count + 1`
done > output/SequentialTimeOutputs.txt

echo "The times have been written to time SequentialTimeOutput.txt" 
